...
def abruptindex(std, mean, sum_vector):
    #creando el mask vector para evitar el marron
    maskvector = np.full(len(sum_vector), True)
    for i in range(10):
        maskvector[i] = False
        sum_vector = sum_vector[maskvector]
...
def whitenImg(filtered, cut_index):
    rows = filtered.shape[0]
    columns = filtered.shape[1]
...
sum_vector = whitenVector(filtered)

print("Soy el sum_vector: ", sum_vector)

print("Soy el size de sum_vector: ", len(sum_vector))

std = np.std(sum_vector) 

print("Soy el std: ", std)

mean = np.mean(sum_vector) 

print("Soy el mean: ", mean)

cut_index = abruptindex(std,mean,sum_vector)

print("Soy el cut_index: ", cut_index)

whitenImg(filtered, cut_index)

#findKeys(filtered)

#idea para poder escoger los indices que quiero del numpy array:
#puedo usar un mask ejemplo: x = np.arrange(9).reshape(3,3) 
#El codigo de arriba me da un arreglo 3x3 del 0-8 
#Mask: mask = np.array({[False, False, False],[True, True, True],...] 
#El mas hace que los tres primeros columns no salgan, que puedo hacerlo 
#para los indices de marron 
